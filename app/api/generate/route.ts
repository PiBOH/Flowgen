  ...
  switch (provider) {
    case 'openrouter': {
      const headers: Record<string, string> = {
        ...
        Authorization: `Bearer ${body.apiKey || process.env.OPENROUTER_API_KEY || ''}`,
      };
...
export async function POST(req: NextRequest) {
  try {
      ...
      return NextResponse.json(
        {
          code: 'FG-002',
          error:
            'API key is missing. Add it in the Settings panel or set OPENROUTER_API_KEY in .env (or .env.local for local overrides).'
        },
        { status: 400 }
      );
    }
    ...
    if (body.jsonMode || body.provider === 'openai') {
